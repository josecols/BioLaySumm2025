...
def calc_alignscore(preds, docs):
    model_path = hf_hub_download(
        repo_id="yzha/AlignScore", filename="AlignScore-base.ckpt"
    )
...
def cal_similarity(preds, refs):
    ...
    scores = [
        np.array(model.similarity(model.encode(p), model.encode(r)))
...
def final_score(task_name):
    ...
    if task_name == "lay_summ":
        ...
        final_scores = {
            key: np.mean([elife_scores[key], plos_scores[key]])
            for key in elife_scores.keys()
        }
    else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Evaluate medical text generation outputs."
    )
    parser.add_argument(
        "--prediction_file",
        type=str,
        required=True,
        default="BioLaySumm2025-eLife_result.json",
